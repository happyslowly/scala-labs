...
class Euro(val euro: Int, val cents: Int = 0, symbol: String = "EUR") extends Currency(symbol) with Ordered[Euro] {
  val inCents: Int = euro * 100 + cents

  def +(that: Euro): Euro = Euro.fromCents(inCents + that.inCents)

  def *(n: Int): Euro = Euro.fromCents(inCents * n)

  override def toString: String = if (cents != 0) f"$symbol: $euro,$cents%02d" else s"$symbol: $euro,--"

  override def compare(that: Euro): Int = this.inCents - that.inCents
}

object Euro {
  implicit class EuroInt(n: Int) {
    def *(euro: Euro): Euro = euro * n
  }
  def fromCents(cents: Int): Euro = new Euro(cents / 100, cents % 100)
}

class Dollar(val dollar: Int, val cents: Int = 0, symbol: String = "EUR") extends Currency(symbol) {
  val inCents: Int = dollar * 100 + cents
}

object Dollar {
  def fromCents(cents: Int): Dollar = new Dollar(cents / 100, cents % 100)
  implicit def dollarToEuro(dollar: Dollar)(implicit converter: CurrencyConverter = DefaultCurrencyConverter): Euro =
